...
async function createStatsByWeekMenu(ctx: MyContext, page = 1) {
  ...
  const workouts = await getUserWorkoutsGroupedByWeek(ctx.dbchat._id.toString(), Number(page), WEEK_PAGE_SIZE);
...
async function createStatsByMonthMenu(ctx: MyContext, page = 0) {
  const { isMetric } = ctx.dbchat.settings;
  const { exercises } = ctx.dbchat;
  const workoutsGroupedByMonth = await getUserWorkoutsGroupedByMonth(ctx.dbchat._id.toString(), Number(page), MONTH_PAGE_SIZE);
...
async function createStatsByDayMenu(ctx: MyContext, page = 1) {
  ...
  if (page && Number(page) > 1) {
    workouts = await getArchivedWorkouts(ctx.dbchat._id.toString(), page, DAY_PAGE_SIZE);
  }
